...
const NAV = [
  { section: 'My Workspace', items: [
    ...
    { to: '/wfh', label: 'Work From Home', icon: '🏠' },
    { to: '/regularization', label: 'Regularization', icon: '✎' },
    { to: '/profile', label: 'My Profile', icon: '👤' },
  ]},
  { section: 'Team', roles: ['manager', 'hr', 'admin'], items: [
    { to: '/approvals', label: 'Approvals', icon: '✔' },
    { to: '/team', label: 'Team Attendance', icon: '👥' },
    { to: '/team-calendar', label: 'Team Calendar', icon: '📅' },
  ]},
  { section: 'HR', roles: ['hr', 'admin'], items: [
    { to: '/employees', label: 'Employees', icon: '🗂' },
    { to: '/policies', label: 'Policies & Holidays', icon: '📋' },
  ...
  return (
    <div className="bell-wrap" ref={ref}>
      <button className="bell" onClick={() => setOpen(!open)} aria-label="Notifications">
        🔔{data.unread > 0 && <span className="dot">{data.unread}</span>}
      </button>
      {open && (
  ...
  return (
    <div className="shell">
      ...
      <aside className={`sidebar${menuOpen ? ' open' : ''}`}>
        <div className="logo"><span className="cube">◈</span> Tesseract HRMS</div>
        <nav>
